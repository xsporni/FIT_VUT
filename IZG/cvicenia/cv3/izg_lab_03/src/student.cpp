...
void pinedaPolygon(const Point *points, const int size, const RGBA &color1, const RGBA &color2)
{
	...
	//////// DOPLNTE KOD /////////
	int minimalniX = points[0].x;
	int maximalniX = points[0].x;
	int minimalniY = points[0].y;
	int maximalniY = points[0].y;

	for (int i = 1; i < size; i++)
	{
		if (points[i].x < minimalniX)
		{
			minimalniX = points[i].x;
		}
		if (points[i].x > maximalniX)
		{
			maximalniX = points[i].x;
		}
		if (points[i].y < minimalniY)
		{
			minimalniY = points[i].y;
		}
		if (points[i].y > maximalniY)
		{
			maximalniY = points[i].y;
		}
	}
	...
	//////// DOPLNTE KOD /////////
	minimalniX = MAX(0, minimalniX);
	minimalniY = MAX(0, minimalniY);

	maximalniX = MIN(width - 1, maximalniX);
	maximalniY = MIN(height - 1, maximalniY);
	...
	//////// DOPLNTE KOD /////////
	EdgeParams hrany(size);
	for (int i = 0; i < size; i++)
	{
		int dalsi = (i + 1) % size;
		hrany[i].deltaX = points[dalsi].x - points[i].x;
		hrany[i].deltaY = points[dalsi].y - points[i].y;
	...
	//////// DOPLNTE KOD /////////
	EdgeFncValues edge_fnc_values(size);
	for (int i = 0; i < size; i++)
	{
		edge_fnc_values[i] = (minimalniY - points[i].y)*hrany[i].deltaX - (minimalniX - points[i].x)*hrany[i].deltaY;
	}
	...
	//////// DOPLNTE KOD /////////
	for (int y = minimalniY; y <= maximalniY; y++)
	{
		bool even = (y - minimalniY) % 2 == 0;
		for (int x = ((even) ? minimalniX : maximalniX); (even) ? (x <= maximalniX) : (x >= minimalniX); x += (even) ? 1 : -1)
		{

			bool je_vacsie_ako_nula = true;
			for (int i = 0; i < size; i++)
			{
				if (edge_fnc_values[i] < 0)
				{
					je_vacsie_ako_nula = false;
					break;
				}
			}
			if (je_vacsie_ako_nula)
			{
				putPixel(x, y, color1);
			}
			if (!((even && x == maximalniX) || (!even && x == minimalniX)))
			{
				for (int i = 0; i < size; i++)
				{
					edge_fnc_values[i] += (even) ? -hrany[i].deltaY : hrany[i].deltaY;
				}
		...
		for (int i = 0; i < size; i++)
		{
			edge_fnc_values[i] += hrany[i].deltaX;
